...
todos:
  - id: flatten-examples
    ...
    status: pending
  - id: worker-facade
    content: facade 增加 opts.useWorker：Worker 持 WASM，主线程 blit/事件/tooltip；默认仍主线程
    status: pending
  - id: worker-large-examples
    content: 仅 bar-large / scatter-large 等卡死例开 useWorker，验证不再页面无响应
    status: pending
  - id: docs-agent
    content: AGENT.md 写清独立示例写法、useWorker 与 function option 不能跨线程克隆
    status: pending
isProject: false
---
...
flowchart LR
  ...
  Facade -->|"setOption cloneable option"| Wasm
  Facade -->|"pointer x y"| Wasm
  Wasm -->|"RGBA or ImageBitmap"| Facade
...
同步 [AGENT.md](AGENT.md)：示例不再经 `runOfficialExample`；`init` 的 `useWorker`；与官方不一致表加 Worker 函数 option 限制。
