...
SVG 地图源仍未实现（`geo-svg-*` / `geo-beef-cuts` 等 probe 判 `ok` 因 `setOption` 不抛，底图为空）。`map: 'china'` / `'world'` 未预注册（`geo-lines` / `lines-airline` 等同样 `ok`、底图空）。不在 runtime 里假实现 `bmap` / `d3`。
