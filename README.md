...
- **未实现（按 canvas 全量对齐计划补齐，不是永久例外）**：已导出只 `console.warn` 的有 `registerLocale`。`smoothMonotone`、完整 SeriesData、pie 完整 `avoidLabelOverlap` / `padAngle`、bar `large`（官网 `bar-large` 50 万柱会卡死）、scatter 统计插件 `ecStat` 与 `scatter-weibo` 海量点卡死、`candlestick-large`（20 万 OHLC 卡死）、`heatmap-bmap` 无百度地图扩展、treemap 大树 / `levels` 色映射 WASM panic、`parallel-nutrients` 约 1.4 万条折线无 `progressive` 会卡死，见 [AGENT.md](AGENT.md) 与 [echarts 文档](wasm-echarts-rs/site/echarts/docs/index.html)。未识别的 `series.type` 会 warn，不再静默忽略。官网 bar 46 + pie 19 + scatter 36 + candlestick 10 + boxplot 4 + heatmap 7 + pictorialBar 8 + gauge 12 + radar 5 + funnel 4 + chord 4 + sunburst 7 + tree 7 + treemap 7 + graph 13 + sankey 7 + themeRiver 2 + calendar 9 + matrix 14 + parallel 4 已进画廊（第 8.1 波 probe 64/65 ok；第 8.2 波 scatter 29/36 ok；第 8.3 波 27/29 ok；第 8.4 波只同步未跑 probe；第 8.5 波 probe 48/51 ok；第 8.6 波 probe 26/27 ok）。
