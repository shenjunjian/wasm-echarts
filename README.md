...
- **未实现（按 canvas 全量对齐计划补齐，不是永久例外）**：已导出只 `console.warn` 的有 `registerPreprocessor` / `registerLocale` / `setPlatformAPI`。其余 chart（Radar/Map/Gauge 等 series）、`smoothMonotone` 见 [AGENT.md](AGENT.md) 与 [echarts 文档](wasm-echarts-rs/site/echarts/docs/index.html)。
